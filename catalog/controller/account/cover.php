<?php
class ControllerAccountCover extends Controller {
	private $error = array();
    ...
    $this->document->setTitle($this->language->get('heading_title'));
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/cover.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/account/cover.tpl';
		} else {
			$this->template = 'default/template/account/cover.tpl';
		}
	...
	public function save(){
        ...
        'success' => 'not ok',
        'message' => 'Change cover unsuccessfully: width is empty !'
      )));
		}
		if ( empty($this->request->post['image']) ){
			return $this->response->setOutput(json_encode(array(
        'success' => 'not ok',
        'message' => 'Change cover unsuccessfully: image is empty !'
      )));
		}
    ...
    if ( !$return ){
    	return $this->response->setOutput(json_encode(array(
        'success' => 'not ok',
        'message' => 'Change cover unsuccessfully !'
      )));
    }
    return $this->response->setOutput(json_encode(array(
      'success' => 'ok',
