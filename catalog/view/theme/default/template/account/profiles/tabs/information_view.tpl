{% block profiles_tabs_information_view %}
<div class="fl profile-column profile-column-information">
	<h3 class="profile-column"><i class="icon-list"></i> Personal Information</h3>
	<div class="profile-column-wrapper">
		<div class="profile-column-content">
			<div class="row-fluid">
				<div class="span2 offset1 profile-label">Username</div>
				<div class="span9 profile-value">wmthiet</div>
			</div>
			<div class="row-fluid">
				<div class="span2 offset1 profile-label">Fullname</div>
				<div class="span9 profile-value">Ngo Van Thiet</div>
			</div>
			<div class="row-fluid">
				<div class="span2 offset1 profile-label">Email</div>
				<div class="span9 profile-value profile-emails">
					<div class="email-item email-primary">
						<span class="label label-success"><i class="icon-envelope"></i></span>
						<span class="">nvthiet.khtn08@gmail.com</span> 
					</div>
					<div class="email-item">
						<span class="label"><i class="icon-envelope"></i></span>
						<span class="">nvthiet.111@gmail.com</span> 
					</div>
					<div class="email-item">
						<span class="label"><i class="icon-envelope"></i></span>
						<span class="">nvthiet.222@gmail.com</span> 
					</div>
				</div>
			</div>
			<div class="row-fluid">
				<div class="span2 offset1 profile-label">Phone</div>
				<div class="span9 profile-value profile-phones">
					<div class="phones-item">
						<span><i class="icon-phone"></i></span>
						<span>098721212</span>
					</div>
					<div class="phones-item">
						<span><i class="icon-phone"></i></span>
						<span>0987232322</span>
					</div>
					<div class="phones-item">
						<span><i class="icon-phone"></i></span>
						<span>0987232324</span>
					</div>
				</div>
			</div>
			<div class="row-fluid">
				<div class="span2 offset1 profile-label">Sex</div>
				<div class="span9 profile-value">Male</div>
			</div>
			<div class="row-fluid">
				<div class="span2 offset1 profile-label">Birthday</div>
				<div class="span9 profile-value">26/06/1990</div>
			</div>
			<div class="row-fluid">
				<div class="span2 offset1 profile-label">Address</div>
				<div class="span9 profile-value">52 DatThanh Street, Ward 6, Tan Binh District
				</div>
			</div>
			<div class="row-fluid">
				<div class="span2 offset1 profile-label">Living</div>
				<div class="span9 profile-value">Ho Chi Minh City</div>
			</div>
			<div class="row-fluid">
				<div class="span2 offset1 profile-label">Industry</div>
				<div class="span9 profile-value">Information Technology</div>
			</div>
		</div>
	</div>
</div>
{% endblock %}

{% block profiles_tabs_information_view_javascript %}
{% endblock %}