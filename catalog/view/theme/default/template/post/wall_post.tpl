...
{% block wall_post_block_javascript %}
    ...
    jQuery(document).ready(function($) {
        function WallPostViewModel(options) {
            var that = this;
            // PROPERTIES
            ...
            function _addPost(post) {
                ...
                $(that.target).gridalicious('append', $(that.target).find('.loaded .gridalicious-item'));
            }
            function _load() {
                //Call common ajax Call:
                $.ajax({
                    url: 'services/get/user/' + that.userSlug + '/posts/' + that.currentPage() + '/',
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        limit: that.limit
        ...
        var oWallPost = new WallPostViewModel({
            target: '.wall-post-list',
            userSlug: currUser.slug,
            urls: {
                    loadNews : { name: "ApiGetUserPost",  params: { user_slug : currUser.slug } },
                },
            });
        ko.applyBindings(oWallPost);
    });
    //--></script>
{% endblock %}