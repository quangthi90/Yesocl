...
(function($, ko, window, Y, undefined) {	
	...
	var CommonFunctions = {
		initZoomImageEvent: function (el){
		...
		sendFriendRequest : function(data, success, fail){
			var ajaxOptions = {
	...
	*/
	function initBindingHanler(){
		...
		ko.bindingHandlers.seeMore = {
		    init: function (element, valueAccessor, allBindingsAccessor) {
		        CommonFunctions.initSeemore($(element));
		    },
		    update: function (element, valueAccessor, allBindingsAccessor) {
		        CommonFunctions.initSeemore($(element));
		    }
		...
		ko.bindingHandlers.emoticon = {
		    init: function (element, valueAccessor, allBindingsAccessor) {
		        CommonFunctions.initEmoticons($(element));
		    }
		};
		...
		ko.bindingHandlers.zoomImageInContent = {
		    init: function (element, valueAccessor, allBindingsAccessor) {
		        ...
		        imgList.each(function(){
		            CommonFunctions.initZoomImageEvent($(this));
		        });
		    },
		    update: function (element, valueAccessor, allBindingsAccessor) {
		...
		ko.bindingHandlers.loadImage = {
		    init: function (element, valueAccessor, allBindingsAccessor) {
		        CommonFunctions.initZoomImageEvent($(element));
		    },
		    update: function (element, valueAccessor, allBindingsAccessor) {
		        CommonFunctions.initZoomImageEvent($(element));
		    }
		...
		ko.bindingHandlers.galleryImage = {
			init: function (element, valueAccessor, allBindingsAccessor) {
		        CommonFunctions.initImageGallery($(element));
		    },
		    update: function (element, valueAccessor, allBindingsAccessor) {
		        CommonFunctions.initImageGallery($(element));
		    }
	...
	function initComponents(){
		ko.components.register('ybutton-friend', {
		    viewModel: function(params) {
		    	var self = this;
		    	self.friendStatus = ko.observable(params.friendStatus || 0);
		    	self.followStatus = ko.observable(params.followStatus || 0);
		    	self.userSlug = params.userSlug || "";

		    	self.addFriend = function(){
		    		var ajaxOptions = {
						url : Y.Routing.generate("ApiPutMakeFriend", {
							user_slug: self.userSlug
						})
					};
					var successCallback = function(data){
						if(data.success === "ok"){
							self.friendStatus(data.status);
						}else{
							//Not OK
						}
					};
		    	...
		    	self.cancelRequest = function(success, fail){
					var ajaxOptions = {
				...
				self.follow = function(success, fail){
					var ajaxOptions = {
				...
				self.unFollow = function(success, fail){
					var ajaxOptions = {
		    ...
		    template:  { element : "template-yFriendButton" }
		});
	}
