...
class ModelGroupAction extends Model {
	public function addAction( $data = array() ) {
		...
		$config = new Config();
		$config->setKey( $this->config->get( 'group_action_title' ) . $data['code'] );
	...
	public function editAction( $id, $data = array() ) {
		...
		$action = $this->dm->getRepository('Document\Group\Action')->find( $id );
	...
	public function deleteAction( $data = array() ) {
		if ( isset($data['id']) ) {
			foreach ( $data['id'] as $id ) {
				...
				$config = $this->dm->getRepository('Document\Setting\Config')->findOneByKey( $this->config->get( 'group_action_title' ) . $action->getCode() );
				if ( $config ){
	...
	public function getAction( $action_id ) {
		return $this->dm->getRepository( 'Document\Group\Action' )->find( $action_id );
	}
	...
	public function getTotalActions() {
		$actions = $this->dm->getRepository( 'Document\Group\Action' )->findAll();

		return count($actions);
	}

	public function getActionByCode( $code ){
		return $this->dm->getRepository( 'Document\Group\Action' )->findBy( array('code' => $code) );
	}
}
?>