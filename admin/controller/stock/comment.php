<?php
class ControllerStockComment extends Controller {
	private $error = array( );
	private $limit = 10;
	private $route = 'stock/comment';
	...
	public function index(){
		...
		$this->load->language( 'stock/comment' );		
		
		$this->load->model( 'stock/comment' );
	...
	public function insert(){
		...
		if ( ($this->request->server['REQUEST_METHOD'] == 'POST') && $this->isValidateForm() ){
			if ( $this->model_stock_comment->addComment( $this->request->get['post_id'], $this->request->post ) ){
				$this->session->data['success'] = $this->language->get( 'text_success' );
			}else{
				$this->session->data['error_warning'] = $this->language->get('error_insert');
			}
			$this->redirect( $this->url->link( 'stock/comment', 'token=' . $this->session->data['token'] . $url, 'SSL' ) );
		}
		
		$this->data['action'] = $this->url->link( 'stock/comment/insert', 'token=' . $this->session->data['token'] . $url, 'SSL' );
	...
	public function update(){
		...
		// request
		if ( ($this->request->server['REQUEST_METHOD'] == 'POST') && $this->isValidateForm() ){
			if ( $this->model_stock_comment->editComment( $this->request->get['post_id'], $this->request->get['comment_id'], $this->request->post ) ){
				$this->session->data['success'] = $this->language->get( 'text_success' );
			}else{
	...
	public function delete(){
		...
		if ( ($this->request->server['REQUEST_METHOD'] == 'POST') && $this->isValidateDelete() ){
			$this->model_stock_comment->deleteComment( $this->request->get['post_id'], $this->request->post );
			$this->session->data['success'] = $this->language->get( 'text_success' );
			$this->redirect( $this->url->link( 'stock/comment', 'token=' . $this->session->data['token'] . $url, 'SSL' ) );
		}
	...
	private function getList( ){
		...
		// breadcrumbs
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link( 'stock/post', 'stock_id=' . $this->request->get['stock_id'] . '&token=' . $this->session->data['token'], 'SSL' ),
      		'separator' => ' :: '
   		);
   		$this->data['breadcrumbs'][] = array(
       		'text'      => $this->language->get( 'heading_title' ),
			'href'      => $this->url->link( 'stock/comment', 'post_id=' . $post->getId() . '&token=' . $this->session->data['token'], 'SSL' ),
      		'separator' => ' :: '
   		);
		...
		// Link
		$this->data['insert'] = $this->url->link( 'stock/comment/insert', 'token=' . $this->session->data['token'] . $url, 'SSL' );
		$this->data['delete'] = $this->url->link( 'stock/comment/delete', 'token=' . $this->session->data['token'] . $url, 'SSL' );
		$this->data['back'] = $this->url->link( 'stock/post', 'stock_id=' . $this->request->get['stock_id'] . '&token=' . $this->session->data['token'], 'SSL' );
		...
		if ( $comments ){
			...
			for ( $i = $comment_total - (($page - 1) * $this->limit) - 1; $i >= $comment_total - ($page * $this->limit) && $i >= 0; $i-- ){
				...
				$action[] = array(
					...
					'href' => $this->url->link( 'stock/comment/update', 'token=' . $this->session->data['token'] . $url . '&comment_id=' . $comments[$i]->getId(), 'SSL' ),
					'icon' => 'icon-edit',
				);
		...
		$pagination->url = $this->url->link('stock/comment', 'token=' . $this->session->data['token'] . '&page={page}' . $url, 'SSL');
			
		$this->data['pagination'] = $pagination->render();

		$this->template = 'stock/comment_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function getForm(){
		...
		if ( !$post ){
			$this->session->data['error_warning'] = $this->language->get('error_post');
			$this->redirect( $this->url->link('stock/post', 'token=' . $this->session->data['token'] . '&stock_id=' . $this->request->get['stock_id'], 'SSL') );
		}
		...
		// breadcrumbs
   		...
   		$this->data['breadcrumbs'][] = array(
       		'text'      => $this->language->get( 'text_post' ),
			'href'      => $this->url->link( 'stock/post', 'stock_id=' . $stock_id . '&token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
   		$this->data['breadcrumbs'][] = array(
       		'text'      => $this->language->get( 'heading_title' ),
			'href'      => $this->url->link( 'stock/comment', 'token=' . $this->session->data['token'] . $url, 'SSL' ),
      		'separator' => ' :: '
   		);
		...
		// Link
		$this->data['cancel'] = $this->url->link( 'stock/comment', 'token=' . $this->session->data['token'] . $url, 'SSL' );
		...
		if ( isset( $this->request->get['comment_id']) ){
			...
			$this->data['action'] = $this->url->link( 'stock/comment/update', 'token=' . $this->session->data['token'] . $url, 'SSL' );
		}
		...
		$this->template = 'stock/comment_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
